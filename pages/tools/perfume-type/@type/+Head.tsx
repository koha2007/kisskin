...
export default function Head() {
  ...
  if (!t) return null
  const title = `${t.koName} 향수 타입 — 추천 향수·메이크업·상황 가이드`
  const desc = `${t.koName} 향수 타입 가이드 — 추천 향수·어울리는 메이크업·계절·상황까지. 무료 1분 진단.`
  const url = `https://kissinskin.net/tools/perfume-type/${t.slug}/`
  const enUrl = `https://kissinskin.net/en/tools/perfume-type/${t.slug}/`
  return (
    <>
      <title>{title}</title>
