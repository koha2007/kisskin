...
### ⭐ 정정 — 막힌 건 텍스트가 아니라 **검색 그라운딩**이다 (8/9 CI 로그 실측)

처음엔 "무료 키엔 쿼터가 아예 없다"로 읽었는데, 성공한 실행 로그가 더 정확한 그림을 줬다:

```
뉴스 생성 (grounded: true)  → Gemini 전부 429 → OpenAI 폴백
EN 번역   (grounded: false) → ✅ 모델 gemini-3.6-flash 사용
```

**같은 키, 같은 모델인데 그라운딩을 붙인 요청만 429 다.**
즉 무료 티어에서 0 인 것은 텍스트 생성이 아니라 **Google Search 그라운딩 쿼터**다.
(공식 가격표의 "3.x 월 5,000건 무료 검색"은 이 신규 키에는 적용되지 않았다.)

실무적 함의:
- 번역처럼 그라운딩이 필요 없는 호출은 **무료 키로 계속 Gemini 에서 돈다**(원가 0)
- 검색이 필요한 뉴스·제품 발굴만 OpenAI 로 넘어간다 — 지금 코드가 정확히 그렇게 동작 중
- 카드가 붙어 유료가 되면 그라운딩이 살아나면서 **전부 Gemini 로 자동 복귀**한다

👉 다음에 같은 사고가 나면: 무료 키 발급은 **번역 등 비그라운딩 호출엔 유효**하지만
   **뉴스 발행은 못 살린다.** 곧장 OpenAI 폴백을 확인할 것.
...
| 항목 | 상태 | 근거 |
|---|---|---|
| 뉴스·제품 **발굴**(그라운딩 필요) | 🟢 **OpenAI `web_search` 로 발행 중** | Gemini 그라운딩 쿼터 0 → 429 |
| EN 번역 등 **비그라운딩** 호출 | 🟢 **무료 Gemini 키로 그대로** | CI 로그 `모델 gemini-3.6-flash 사용` |
