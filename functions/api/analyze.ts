...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    // 3x3 그리드는 항상 정사각형으로 생성 (행별 줌 레벨 불일치 방지)
...
${skinDesc} 총 9가지 메이크업을 3x3 그리드(정사각형 이미지)로 생성해줘.

[최우선 규칙 - 얼굴 크기/위치 완전 고정]
- 9칸 모두 원본 사진과 100% 동일한 얼굴이어야 해.
- 얼굴 위치, 크기, 각도, 방향, 표정이 9칸 전부 완전히 동일해야 해. 마치 같은 사진을 복사해서 메이크업만 바꾼 것처럼.
- 절대 금지: 아래쪽 행으로 갈수록 얼굴이 확대(줌인)되는 것. 1행, 2행, 3행의 얼굴 크기가 100% 동일해야 해.
- 절대 금지: 어떤 칸에서든 머리 꼭대기나 이마가 잘리는 것. 9칸 모두 머리 전체가 보여야 해.
- 눈, 코, 입, 턱, 이마, 귀, 헤어라인의 픽셀 위치가 9칸 모두에서 정확히 같아야 해.
- 얼굴 형태, 이목구비, 피부색, 머리카락 스타일을 절대 변경하지 마.
- 오직 화장(아이섀도, 립스틱, 블러셔, 파운데이션 등)만 다르게 적용해.

[그리드 규칙]
- 정사각형 이미지를 정확히 3x3 = 9개의 동일한 정사각형 칸으로 나눠.
...
[프레이밍 규칙]
- 9칸 모두 동일한 프레이밍: 얼굴이 각 정사각형 칸의 중앙에 위치하고, 이마 위 여유공간부터 턱 아래까지 모두 보여야 해.
- 머리 꼭대기가 절대 잘리면 안 돼. 이마 위에 약간의 여백을 유지해.
- 모든 칸에서 얼굴이 칸 높이의 약 75-80%를 차지하게 해. (나머지는 머리 위 여백과 턱 아래 여백)
...
핵심 요약: 원본 사진을 9번 복사한 뒤 각각 다른 메이크업만 입힌 것처럼 만들어. 얼굴 위치와 크기와 줌 레벨은 9칸 전부 완전히 동일해야 해. 특히 3행(7,8,9번)이 1행(1,2,3번)보다 확대되면 절대 안 돼.`)
