...
2. 각 칸마다 얼굴이 머리 꼭대기, 이마, 턱, 귀, 머리카락이 절대 잘리지 않게 해. 얼굴 위아래좌우 모두 넉넉한 여백을 둬.
3. 전체 3x3 그리드 바깥에도 위, 좌, 우에 균등한 여백(패딩)을 넣어서 그리드가 이미지 가장자리에 붙지 않게 해줘.
4. 각 칸의 구조: 위쪽은 얼굴 사진 영역, 아래쪽에 흰색 또는 밝은 회색 배경의 텍스트 영역을 만들어서 메이크업 스타일 이름을 깔끔한 검은색 텍스트로 표시해줘. 사진 위에 텍스트를 겹치지 말고, 사진 아래 별도 영역에 배치해.
5. 텍스트는 영어로 정확하게 작성하고 오타 없이 해줘.
6. 그리드 라인을 얇고 명확하게 표시해줘.
7. 9칸 모두 동일한 크기로 균등하게 배치해줘.
8. 총 9가지 메이크업 스타일을 확실하고 정확하게 자연스럽게 표현해줘.

9가지 메이크업과 각 칸 아래에 표시할 영어 이름 (좌→우, 위→아래 순서):
1번: 자연스러운 광채 메이크업 → "Natural dewy glow"
2번: 뽀얀 구름같은 클라우드 스킨 메이크업 → "Cloud skin makeup"
3번: 진한 입술 강조 블러드 립 메이크업 → "Blood lip makeup"
4번: 화려한 눈 강조 맥시멀리스트 아이 메이크업 → "Maximalist eye makeup"
5번: 금속 광택 메탈릭 아이 메이크업 → "Metallic eye makeup"
6번: 강렬한 입술 볼드 립 메이크업 → "Bold lip makeup"
7번: 블러쉬 드레이핑 메이크업 → "Blush draping makeup"
