...
1: 컨실러와 틴티드 SPF로 피부만 깨끗하게 정돈, 화장한 티 없이 수분감과 윤기를 살린 건강한 빛
2: 틴티드 선스크린, 세럼 파운데이션 등 스킨케어 겸용 베이스, 보습감 있는 투명한 피부 표현
3: 안쪽부터 자연스럽게 번지듯 색을 넣는 립, 정교한 립라인 없이 은은한 혈색감
4: 블랙이나 다크브라운 아이섀도를 눈꼬리에 스머지, 어둡고 날카로운 스모키 눈매
5: 브론즈 등 하나의 컬러 계열로 눈썹·아이섀도·입술을 통일, 텍스처 차이로 세련미
6: 컬러 보정 프라이머로 칙칙함을 잡고 눈밑 다크서클을 커버한 깔끔한 기능성 그루밍 메이크업
7: 눈꼬리나 언더라인에 블루·실버·터콰이즈 등 포인트 컬러, 프로스티드 텍스처
8: 창백하게 정돈된 피부 위에 강렬한 스모키 아이와 딥 레드 톤
9: 글로우 베이스 위 섬세한 아이섀도, 애교살 강조, 정교한 눈썹 정리, 컬러 립밤 풀세트 연출
