...
- 사람의 얼굴 형태, 이목구비, 피부색, 피부 밝기, 인종, 얼굴 윤곽, 눈코입 위치와 크기를 절대 절대 변경하지 마. 원본 얼굴과 100% 동일해야 해
...
[9가지 남성 메이크업 - 좌→우, 위→아래 순서 / 각 스타일을 과감하고 확실하게 표현]
...
4: 그런지 스모키 아이 - 눈 주변 전체를 블랙+다크브라운으로 진하고 거칠게 스머지. 언더아이까지 어둡게. 날카롭고 반항적인 느낌. 입술은 어두운 베리톤. 피부색은 원본과 동일하게 유지
...
7: 블루 컬러 포인트 아이 - 선명한 블루 아이섀도를 눈두덩이에 확실하게 표현. 실버 글리터 포인트. 피부색은 원본과 반드시 동일하게 유지하고 블루 컬러만 눈에 적용
8: 뱀파이어 로맨틱 - 눈 주변은 진한 버건디+다크브라운 스모키. 입술은 짙은 다크레드/와인색. 신비로운 분위기. 피부색은 원본 유지
9: K-팝 아이돌 - 완벽한 글로우 베이스, 반짝이는 핑크+피치 아이섀도, 선명한 애교살 하이라이트, 코랄핑크 그라데이션 립, 눈썹 정교하게 정리. 머리카락을 트렌디한 색(애쉬 블론드/실버그레이/밀크브라운 중 택1)으로 변경. 옷은 원본 그대로 유지

핵심: 9칸 모두 원본과 동일한 피부색·모자·옷·액세서리를 유지해야 해! 피부색이 달라지면 완전한 실패야! 메이크업만 다르게! 특히 1번(민낯)과 2번(물광)은 피부 질감으로, 3번은 립 컬러로, 4번은 진한 스모키로, 5번은 통일 컬러로, 6번은 컨투어링으로, 7번은 블루 아이섀도로, 8번은 다크 립+버건디 아이로 차이를 확실하게 내야 해. 머리카락 색 변경은 오직 9번에서만! 이미지 위에 어떤 글자도 렌더링하지 마.`
