...
🚨🚨🚨 최우선 규칙 2 — 남성 메이크업 강도 제한 (가장 중요!) 🚨🚨🚨
- 남성 메이크업의 핵심은 "안 한 듯 세련되게"
- 여성 메이크업의 30~40% 강도가 남성 메이크업의 최대치. 절대로 여성 메이크업 수준으로 진하게 하면 안 됨
- 아이섀도: 눈두덩에 살짝 얹는 수준. 색이 은은하게 비치는 정도. 눈 아래/볼까지 절대 번지지 않게
- 립: 입술 원래 색에서 약간 색감이 더해진 수준. 틴트를 살짝 바른 느낌. 진한 립/블랙 립/고스 립 절대 금지
- 블러셔: 거의 안 보이게 은은하게. "볼에 색이 있다"고 느끼면 이미 과한 것
- 아이라인: 굵은 아이라인 금지. 속눈썹 라인을 따라 가늘게만
- 전체적으로 "잘생겨 보이는 피부 관리+약간의 색감" 느낌. 화장한 티가 나면 실패
- 기준점: 한국 남자 배우(박서준, 차은우)가 드라마에서 하는 자연스러운 메이크업 수준
- 9개 스타일 간 차이는 "색상 톤"과 "포인트 부위"로 구분. 강도를 높여서 구분하지 마

⚠️ 텍스트 금지: 이미지에 글자, 숫자, 라벨, 스타일명, 워터마크 등 어떤 텍스트도 절대 넣지 마.
...
1: 노 메이크업 메이크업 - 피부결만 깨끗하게 보정. 눈썹 자연스럽게 정돈. 투명 립밤만. 화장한 티가 전혀 안 나는 룩. 피부가 원래 좋은 사람처럼
2: 스킨케어 하이브리드 베이스 - 촉촉한 윤광 피부. 이마·코끝·광대에 자연스러운 광택(하이라이터가 아니라 수분감). 1번보다 피부에 건강한 윤기. 립은 무색 립밤
3: 블러드 립 - 입술에만 자연스러운 코랄/로즈 틴트. 물들인 듯한 느낌. 입술 안쪽이 살짝 더 진한 그라데이션. 피부는 깨끗한 세미매트
4: 그런지/스모키 아이 - 눈두덩에만 브라운+다크카키를 아주 얇게 블렌딩. 눈매가 깊어 보이는 효과. 눈 아래는 절대 번지지 않게. 립은 자연스러운 누드. 차은우 스모키 수준
5: 모노크롬 - 피치/테라코타 한 톤으로 눈두덩·볼·입술에 은은하게 통일감. 각 부위에 같은 색을 살짝만. 전체적으로 따뜻한 톤이 감도는 느낌
6: 유틸리티 메이크업 - 잡티 없는 완벽한 피부. 눈썹을 또렷하게 정돈(1번보다 선명). 입술은 자연스러운 코랄 틴트 한 톤. 면접/비즈니스에 적합한 깔끔한 룩
7: 블루 포인트 아이 - 눈두덩에 네이비/블루를 아주 얇게 한 겹만. "혹시 파란색인가?" 싶을 정도로 은은하게. 절대로 두껍게 바르지 마. 립은 자연스러운 누드
8: 뱀파이어 로맨틱 - 눈두덩에 버건디/로즈브라운을 얇게. 입술은 자연스러운 로제 틴트. 전체적으로 따뜻한 붉은 기운이 은은하게 감도는 분위기. 세련된 무드
9: K-팝 아이돌 메이크업 - 글로우 베이스. 눈두덩에 핑크+피치 쉬머를 살짝. 코랄핑크 립 틴트. 머리색 변경(애쉬블론드/실버그레이/밀크브라운). 옷 원본 유지. BTS 뷔 수준의 자연스러운 아이돌 메이크업

이 사진을 3x3 그리드로 균등하게 9등분해줘. 사진 해상도 높여줘. 다시 한번 강조: 텍스트 절대 금지. 9개 셀 모두 반드시 원본과 100% 동일한 얼굴 — 메이크업만 다르게. 남성 메이크업은 "안 한 듯 자연스럽게". 여성 메이크업의 30~40% 강도가 최대치.`
