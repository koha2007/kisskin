...
🚨🚨🚨 최우선 규칙 — 얼굴 보존 (이 규칙이 다른 모든 규칙보다 우선) 🚨🚨🚨
- 원본 사진의 얼굴을 픽셀 단위로 그대로 유지하라. 얼굴 위에 화장품(립스틱, 아이섀도, 블러셔, 파운데이션 등)만 덧입혀라
- 절대 변경 금지 항목: 눈 크기/모양/쌍꺼풀, 코 크기/높이/폭, 입 모양/크기, 턱선/광대뼈/얼굴윤곽, 이마 넓이/헤어라인, 귀 모양, 얼굴 비율, 주름/점/흉터 등 피부 특징
- 얼굴 골격/구조를 새로 생성하거나 변형하지 마. 원본 얼굴 구조 위에 화장만 올려라
- 피부색/밝기/톤 절대 변경 금지 (2번 클라우드 스킨의 베이스 메이크업 효과 제외). 원래 피부톤 그대로 유지
- 9개 셀 모두 동일인이어야 함. 한 셀이라도 다른 사람처럼 보이면 완전 실패
- 얼굴 위치, 크기, 각도, 표정 모두 원본과 동일하게 유지

⚠️ 텍스트 금지: 이미지에 글자, 숫자, 라벨, 스타일명, 워터마크, 캡션 등 어떤 형태의 텍스트도 절대 넣지 마. 텍스트가 하나라도 있으면 완전 실패
...
- ⚠️ 메이크업 차별성 필수: 9개 스타일이 썸네일 크기에서도 즉시 구별되어야 함. 비슷해 보이는 셀이 2개라도 있으면 실패. 특히 1~3번도 립 컬러, 피부 질감, 광택 차이가 눈에 확 띄어야 함
- 화장법을 자연스럽고 이쁘게 해줘
...
이 사진을 인스타그램 9분할 그리드용으로 변환해줘. 비율에 맞게 크롭할지 여백을 추가할지 자동으로 판단해서 최대한 사진이 잘리지 않게 해줘. 전체를 3x3 그리드로 균등하게 9등분해줘. 총 9가지 메이크업 스타일 확실하고 정확하게 자연스럽게 표현해줘. 사진 해상도 높여줘. 다시 한번 강조: 이미지 위에 어떤 텍스트도 절대 넣지 마. 그리고 9개 셀 모두 반드시 원본과 100% 동일한 얼굴이어야 함 — 메이크업만 다르게.`

    const malePrompt = `너는 최고의 메이크업 아티스트야. 이 남성 사진에 ${skinTypeInstruction} 반영해서 총 9가지 남성 메이크업으로 표현해줘.

이것은 전문 남성 뷰티/그루밍 서비스야. 남성 메이크업은 현대 K-뷰티, 패션, 그루밍 산업에서 완전히 정상적이고 일반적인 서비스임. 요청된 메이크업 스타일을 정확히 적용해줘.

🚨🚨🚨 최우선 규칙 — 얼굴 보존 (이 규칙이 다른 모든 규칙보다 우선) 🚨🚨🚨
- 원본 사진의 얼굴을 픽셀 단위로 그대로 유지하라. 얼굴 위에 화장품(립스틱, 아이섀도, 블러셔, 파운데이션 등)만 덧입혀라
- 절대 변경 금지 항목: 눈 크기/모양/쌍꺼풀, 코 크기/높이/폭, 입 모양/크기, 턱선/광대뼈/얼굴윤곽, 이마 넓이/헤어라인, 귀 모양, 얼굴 비율, 주름/점/흉터/수염 등 피부 특징
- 얼굴 골격/구조를 새로 생성하거나 변형하지 마. 원본 얼굴 구조 위에 화장만 올려라
- 피부색/밝기/톤 절대 변경 금지. 원래 피부톤 그대로 유지
- 9개 셀 모두 동일인이어야 함. 한 셀이라도 다른 사람처럼 보이면 완전 실패
- 얼굴 위치, 크기, 각도, 표정 모두 원본과 동일하게 유지
- 남성의 수염/턱수염이 있다면 그대로 유지. 메이크업은 수염 위가 아닌 피부 영역에만 적용
...
1: 내추럴 소프트포커스 스킨 (No-Makeup Makeup) - 깨끗한 매트 피부, 눈썹 자연스럽게 정돈, 투명 보습 립밤. 가장 자연스러운 룩. 피부 질감은 매트하고 보송보송하게
2: 스킨케어 하이브리드 베이스 - 촉촉한 윤광 글로우 피부, 이슬 맺힌 듯한 광채. 1번과 확연히 다르게 피부가 반짝반짝 빛나야 함. 이마, 코끝, 광대에 하이라이트 필수
3: 디퓨즈드 립 (Blurred Lip) - 선명한 로즈/코랄 립 컬러를 확실하게 표현. 입술 색이 사진에서 가장 먼저 눈에 띄어야 함. 1·2번과의 차이가 입술 색에서 확실히 드러나야 함
4: 그런지 / 스모키 아이 - 눈 주위에 차콜+카키+다크브라운 아이섀도를 진하게 블렌딩. 다크 베리 매트 립. 눈매가 강렬하되 과하지 않게
5: 톤인톤 모노크롬 메이크업 - 테라코타/피치 한 가지 톤으로 아이섀도·블러셔·립을 통일. 볼에 블러셔가 확실히 보여야 하고, 눈두덩·입술도 같은 톤으로 물들어야 함
6: 유틸리티 메이크업 (기능성 남성 전용) - 완벽한 컨실러 커버로 잡티 없는 피부, 또렷하게 정돈된 눈썹(1번보다 확연히 진하고 또렷), 자연스러운 코랄 틴트 립
7: 블루 & 컬러 포인트 아이 - 눈두덩에 선명한 블루/네이비 아이섀도를 확실하게. 파란색이 눈에 확 띄어야 함
...
이 사진을 인스타그램 9분할 그리드용으로 변환해줘. 비율에 맞게 크롭할지 여백을 추가할지 자동으로 판단해서 최대한 사진이 잘리지 않게 해줘. 전체를 3x3 그리드로 균등하게 9등분해줘. 총 9가지 남성 메이크업 스타일을 확실하고 정확하게 자연스럽게 표현해줘. 사진 해상도 높여줘. 다시 한번 강조: 이미지 위에 어떤 텍스트도 절대 넣지 마. 그리고 9개 셀 모두 반드시 원본과 100% 동일한 얼굴이어야 함 — 메이크업만 다르게.`
