...
⚠️ 가장 중요한 규칙: 이미지에 텍스트를 절대 넣지 마! 글자, 숫자, 라벨, 스타일 이름, 워터마크, 캡션 등 어떤 형태의 텍스트도 이미지 위에 렌더링하지 마. 텍스트가 하나라도 있으면 완전 실패로 간주함. 오직 메이크업된 얼굴 사진만 있어야 함.

[절대 규칙 - 위반 시 실패]
- ⚠️ 얼굴 동일성이 최우선: 9개 셀 모두 원본 사진과 100% 동일한 얼굴이어야 함. 눈 모양, 코 크기, 입 모양, 턱선, 이마, 귀, 얼굴 비율이 한 치도 달라지면 안 됨. 다른 사람처럼 보이면 완전 실패
- 원본 사진의 얼굴을 그대로 복사한 뒤 메이크업만 올려라. 얼굴 구조를 재생성하지 마
...
- 이빨이 보이면 하얗고 깨끗하게
- 텍스트/폰트/글자/숫자/라벨/스타일명/워터마크 절대 금지. 이미지는 순수한 사진만으로 구성
...
이 사진을 인스타그램 9분할 그리드용으로 변환해줘. 비율에 맞게 크롭할지 여백을 추가할지 자동으로 판단해서 최대한 사진이 잘리지 않게 해줘. 전체를 3x3 그리드로 균등하게 9등분해줘. 총 9가지 메이크업 스타일 확실하고 정확하게 자연스럽게 표현해줘. 사진 해상도 높여줘. 다시 한번 강조: 이미지 위에 어떤 텍스트도 절대 넣지 마. 스타일 이름, 번호, 라벨 등 일체 금지. 그리고 9개 셀 모두 반드시 동일한 사람의 얼굴이어야 함.`
