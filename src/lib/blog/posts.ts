...
export const BLOG_POSTS: BlogPost[] = [
  ...
  {
    slug: 'personal-color-system-deep-dive',
    category: 'guide',
    title: '퍼스널 컬러 시스템 완전 분석 — 4계절을 넘어 12가지 톤까지',
    summary:
      '봄웜·여름쿨·가을웜·겨울쿨 4계절은 시작점일 뿐. 각 시즌 안에 라이트·딥·뮤트·비비드 4가지 세부 톤이 있다. 진짜 퍼스널 컬러를 찾는 12톤 시스템을 정리한다.',
    body: [
      '퍼스널 컬러(Personal Color)는 1980년대 캐롤 잭슨의 "Color Me Beautiful"에서 시작된 컬러 분석 시스템이다. 당시는 봄·여름·가을·겨울 4계절 분류였지만, 1990년대 일본·한국에서 12톤 또는 16톤 세부 분류로 정교화됐다.',
      '4계절 → 12톤 진화의 핵심은 명도(밝기)와 채도(선명도)의 추가다. 봄웜은 라이트 스프링·브라이트 스프링·트루 스프링 3톤으로, 여름쿨은 라이트 서머·트루 서머·뮤트 서머 3톤으로 각각 세분화된다. 가을웜은 트루 어텀·뮤트 어텀·딥 어텀, 겨울쿨은 브라이트 윈터·트루 윈터·딥 윈터로 세분화된다. 각 톤마다 어울리는 컬러 팔레트가 미묘하게 다르다.',
      '예를 들어 같은 봄웜이라도 라이트 스프링은 페일 핑크·라이트 코랄·소프트 옐로우가 가장 어울린다. 트루 스프링은 비비드 코랄·골든 옐로우·따뜻한 그린이 어울리고, 브라이트 스프링은 토마토 레드·터키 블루·핫 핑크 같은 채도 높은 컬러가 어울린다. 4계절 분류만으로는 잡히지 않는 미묘한 차이다.',
      '본인 톤을 찾는 가이드라인: (1) 4계절 1차 진단 — 본인의 시즌이 봄·여름·가을·겨울 중 어디에 속하는지. (2) 명도 체크 — 본인 피부톤이 매우 밝다면 라이트, 어둡다면 딥에 가깝다. (3) 채도 체크 — 본인이 화려한 컬러보다 차분한 컬러가 어울린다면 뮤트, 정반대면 비비드/브라이트. (4) 자연광에서 흰 천을 얼굴에 대고 사진을 찍어 컬러군별로 비교.',
      '12톤 시스템의 장점은 정확도, 단점은 복잡성이다. 일반 소비자가 12톤을 모두 구분해 사용하기는 어렵다. 따라서 실용적 접근법: 4계절 + 라이트/뮤트의 6분류 정도로 시작하고, 본인이 컬러에 익숙해지면 점진적으로 세분화하는 것이 좋다.',
      '퍼스널 컬러를 일상에서 활용하는 5가지 원칙: (1) 얼굴 가까이 시즌 컬러를 — 상의·이너·립·블러쉬. (2) 얼굴 멀리는 자유 — 하의·신발·가방. (3) 시즌 컬러는 70%, 도전 컬러는 30% 비율. (4) 비시즌 컬러는 면적을 줄여 — 스카프·악세서리. (5) 시즌이 애매할 때는 인접 시즌의 채도 낮은 컬러로 안전하게.',
    ],
    date: '2026-04-23',
    readMinutes: 7,
    tags: ['퍼스널컬러', '가이드', '컬러분석'],
  ...
  {
    slug: 'face-shape-celebrity-analysis',
    category: 'guide',
    title: '5가지 얼굴형 — 글로벌 셀럽으로 보는 얼굴형별 메이크업 전략',
    summary:
      'oval·round·square·oblong·heart 5가지 얼굴형의 특징을 글로벌 셀럽 사례로 분석. 본인 얼굴형의 메이크업·헤어 전략을 구체적으로 설계한다.',
    body: [
      '얼굴형은 메이크업·헤어·악세서리 선택의 가장 큰 좌표축이다. 5가지 기본 얼굴형(oval·round·square·oblong·heart)을 글로벌 셀럽 사례로 살펴보면 본인 얼굴형의 전략이 명확해진다.',
      'OVAL(타원형) — 가장 균형 잡힌 얼굴형으로 모든 메이크업·헤어가 어울린다. 대표 셀럽: Beyonce, 제니(BLACKPINK), Olivia Wilde. 메이크업 전략: 별도 컨투어가 거의 필요 없음. 오히려 다양한 룩을 시도해도 자연스러움. 헤어 전략: 어떤 길이·스타일도 무난.',
      'ROUND(둥근형) — 부드럽고 어려 보이는 인상. 대표 셀럽: 셀레나 고메즈, 미셸 윌리엄스, 정채연. 메이크업 전략: 광대뼈 옆에 쉐이딩으로 얼굴 길이감을 만들어 인상을 또렷하게. 아이 메이크업은 길게 빼는 캣아이가 어울림. 헤어 전략: 옆머리는 살짝 길게 두고 정수리에 볼륨을 줘 세로 길이감 강조.',
      'SQUARE(각형) — 강하고 명확한 인상. 대표 셀럽: 안젤리나 졸리, 송혜교 일부, 해리 스타일스. 메이크업 전략: 턱 모서리에 쉐이딩, 광대뼈와 이마에 하이라이터로 부드러운 곡선 강조. 컨투어가 가장 효과적인 얼굴형. 헤어 전략: 곡선이 있는 웨이브 스타일이 잘 어울림. 직선 보브는 각형 특징을 더 강조하므로 주의.',
      'OBLONG(긴형) — 세로가 길고 우아한 인상. 대표 셀럽: 사라 제시카 파커, 산드라 오, 강동원. 메이크업 전략: 이마와 턱 끝에 쉐이딩으로 길이감을 줄이고, 광대뼈에 하이라이트로 가로 폭 강조. 블러쉬는 광대 가장 높은 곳에 둥글게. 헤어 전략: 옆 폭을 넓히는 웨이브, 앞머리로 이마를 가리는 스타일.',
      'HEART(하트형) — 광대가 넓고 턱이 좁은 인상. 대표 셀럽: 리즈 위더스푼, 김태리, 카메론 디아즈. 메이크업 전략: 이마 옆에 쉐이딩, 턱 라인에 하이라이트로 V라인을 부드럽게 만듦. 블러쉬는 광대 안쪽에 뭉치지 않게 펴 발라야 자연스럽다. 헤어 전략: 턱 길이 정도의 보브, 또는 옆에 볼륨을 더한 웨이브.',
      '얼굴형 진단의 한계와 활용법: (1) 100% 한 가지 얼굴형인 사람은 거의 없다 — 주된 형태 + 부형으로 이해하자. (2) 얼굴형은 변하지 않지만 인상은 메이크업·헤어로 크게 바꿀 수 있다. (3) 가이드는 어울릴 확률을 높이는 도구이지 절대 규칙이 아니다 — 자신의 얼굴형과 다른 메이크업을 시도해도 충분히 멋질 수 있다.',
    ],
    date: '2026-04-22',
    readMinutes: 7,
    tags: ['얼굴형', '가이드', '셀럽', '메이크업'],
  },
  {
  ...
  {
    slug: 'eyeliner-shape-eye-type',
    category: 'guide',
    title: '눈매별 아이라이너 그리는 법 — 6가지 눈 타입 완전 가이드',
    summary:
      '쌍커풀, 무쌍, 졸린눈, 큰눈, 짝눈, 처진눈. 6가지 눈 타입별로 가장 어울리는 아이라이너 그리는 법을 정리했다.',
    body: [
      '아이라이너는 같은 라인이라도 눈 모양에 따라 결과가 천차만별이다. 셀프 메이크업의 가장 어려운 단계 중 하나로 꼽히는 이유다. 6가지 기본 눈 타입별 아이라이너 가이드를 정리한다.',
      '1. 쌍커풀 (Double Eyelid) — 가장 일반적이고 다양한 라인이 어울리는 눈 타입. 추천: 클래식 캣아이, 일자 라인 모두 OK. 핵심 팁 — 쌍커풀 라인 안쪽까지 라이너가 들어가지 않도록 주의. 눈을 떴을 때 라인이 안 보이지 않게 한 단계 더 위로 올려 그릴 것.',
      '2. 무쌍 (Monolid) — 쌍커풀이 없는 눈. 한국·일본·중국 동아시아 인구의 약 30%. 추천: 두꺼운 일자 라인, 또는 끝을 살짝만 올린 미니 캣아이. 핵심 팁 — 라인을 너무 얇게 그리면 눈을 떴을 때 거의 보이지 않으므로 일반보다 1.5~2배 두껍게. 반대로 너무 두꺼우면 눈이 작아 보일 수 있어 균형이 중요.',
      '3. 졸린눈/처진눈 (Sleepy/Hooded Eye) — 눈꼬리가 처지거나 눈꺼풀이 눈을 살짝 덮는 타입. 추천: 끝을 강하게 올린 캣아이(시메 라인). 핵심 팁 — 라이너 끝 각도가 핵심. 눈꼬리 자연 라인보다 15~30도 올려 그려야 처진 인상을 보완. 너무 길게 빼면 부자연스러움.',
      '4. 큰눈 (Large Eye) — 눈이 크고 또렷한 타입. 추천: 미세한 라인, 또는 라인 없이 마스카라만. 핵심 팁 — 이미 눈이 또렷하므로 강한 라이너는 부담스러울 수 있다. 라이너 대신 다크 브라운 섀도우로 라인을 흐리게 잡으면 자연스럽다.',
      '5. 짝눈 (Asymmetric Eye) — 한쪽 눈과 반대쪽 눈의 크기·모양이 다른 타입. 누구나 약간씩 있다. 추천: 작은 쪽 눈에는 약간 더 두꺼운 라인, 큰 쪽 눈에는 얇은 라인으로 균형. 핵심 팁 — 양쪽을 똑같이 그리려 하지 말고, 거울에서 떨어져 봤을 때 균형이 맞는지를 기준으로.',
      '6. 외꺼풀 + 작은 눈 (Asian Small Eye) — 한국 여성의 약 25%. 추천: 바깥 1/3에 두꺼운 캣아이, 안쪽 2/3는 얇거나 생략. 핵심 팁 — 안쪽까지 라이너를 그리면 눈이 더 작아 보인다. 바깥쪽에만 집중해 시선 유도.',
      '눈매별 공통 가이드: (1) 라이너 도구 — 처음에는 펜슬형, 익숙해지면 펠트팁(액체) 사용. (2) 워터프루프 — 출퇴근·운동·여름철엔 필수. (3) 색깔 — 검정 외에도 다크 브라운, 다크 플럼, 차콜 그레이가 자연스러운 비검정 옵션. (4) 양 눈 동시에 그리기 — 한쪽씩 완성하면 비대칭이 심해진다. 점진적으로 양쪽을 동시에 빌드업.',
    ],
    date: '2026-04-12',
    readMinutes: 7,
    tags: ['아이라이너', '눈매', '가이드'],
