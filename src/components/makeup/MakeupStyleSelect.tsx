...
// 2026-07-27 브랜드 정렬: 보라 그라데이션·글래스 알약을 걷어내고 네이비 단색 위에
// 사진만 색을 갖게 했다. 선택 표시도 흰 링 → 핑크 링(핑크 = 행동·선택).
//
...
import {
  NAVY, PRIMARY, screenBg, surfaceStyle, footerScrim,
  btnPrimary, btnPrimaryStyle, stepBar,
} from './theme'
  ...
  const Card = ({ id }: { id: MakeupStyleId }) => {
    ...
    return (
      <button
        ...
        onClick={() => pick(id)}
        aria-pressed={isSel}
        className={`style-card group relative w-full rounded-lg aspect-[4/5] p-2.5 flex flex-col justify-end text-left overflow-hidden transition-all ${
          isSel
            ? 'ring-2 ring-offset-2 opacity-100'
            : 'ring-1 ring-white/15 opacity-85 hover:opacity-100'
        }`}
        style={{
          background: s.mood,
          ...(isSel ? { '--tw-ring-color': PRIMARY, '--tw-ring-offset-color': NAVY } as React.CSSProperties : {}),
        }}
      >
        ...
        {/* 선택 체크 (우상단 동그라미 체크) */}
        {isSel && (
          <span className="absolute top-1.5 right-1.5 w-5 h-5 flex items-center justify-center text-white" style={{ background: PRIMARY }}>
  ...
  return (
    <div className="min-h-[100dvh] flex flex-col font-display text-white" style={screenBg}>
      ...
      <main className="flex-1 flex flex-col px-5 pt-5 pb-4 max-w-xl w-full mx-auto">
        <h2 className="text-[24px] md:text-[28px] font-extrabold leading-tight tracking-[-0.02em] mb-4 break-keep">
        ...
        {preview && (
          <figure className="relative rounded-2xl overflow-hidden mb-3" style={{ background: selectedStyle.mood }}>
            <img
            ...
            {/* 비포/애프터 토글 */}
            <div className="absolute top-2.5 left-2.5 flex bg-black/55 text-[11px] font-extrabold">
              {[
                ...
                return (
                  <button
                    ...
                    aria-pressed={on}
                    className={`px-3 py-1.5 transition-colors ${on ? 'bg-white' : 'text-white/70'}`}
      ...
      >
        ...
        {loginHref && (
          <a
            href={loginHref}
            className="mb-2.5 flex items-center justify-center gap-1.5 px-3 py-2 text-[12px] font-semibold transition-colors hover:brightness-110"
            style={surfaceStyle}
          >
        ...
        <button onClick={handleConfirm} className={btnPrimary} style={btnPrimaryStyle}>
