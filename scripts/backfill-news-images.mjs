...
async function genImage(openaiKey, slug) {
  let b64
  for (let retry = 0; retry < 3 && !b64; retry++) {
    b64 = await generateImageB64(openaiKey, buildNewsImagePrompt({ slug }, retry), '3:4')
...
async function main() {
  ...
  for (const b of targets) {
    try {
      ...
      const imagePath = await genImage(openaiKey, b.slug)
      let koNote = '(이미 image 있음 — 파일만 갱신)'
      if (!b.hasImage) {
